...
(function () {
  ...
  var CHANGES = [
    ...
    { date: '2026-09-26', tag: 'FEAT',  label: 'Roster-health metrics now AVERAGED over the last several daily scans instead of a single snapshot — activity swings a lot day to day (a big event login mass-refreshes everyone's "last seen"), so Iron Discipline / True Strength / Most Active and the Power Audit's ghost% now use a smoothed multi-scan reading once we have 2+ days of history. Steadier, harder to game, closer to the truth.' },
    { date: '2026-09-26', tag: 'FEAT',  label: 'NEW: Alliance Awards (dashboard 🏆 tab) — auto-generates data-earned awards for every alliance on any warzone: Strongest, The Challenger, Fastest Rising, Elite Squad, Dark Horse, plus Iron Discipline / True Strength / Most Active (these three only for rosters LW Atlas scanned in full, so they can't be gamed). Screenshot-ready to drop in world chat. Also fixed the Power Audit denominator so per-member / effective power now use the authoritative (100-cap) member count instead of the base-scan dot count — kills the bogus "2848M/member" readings on thinly-scanned alliances.' },
    { date: '2026-09-26', tag: 'FEAT',  label: 'New-Arrival Radar now tells transfers apart from farms: 🧭 "transferred-in" (an established player whose base just appeared on the warzone — caught by diffing daily scans, so it finally catches veteran server-hoppers like a new R5) vs 🆕 "new / farm" (an account new to LW Atlas, almost always a migrated-in alt — you can\'t START on an old server). Shows a live count of each. Merges the movement-snapshot detector into the radar so both kinds surface in one list.' },
    { date: '2026-09-24', tag: 'FEAT',  label: 'NEW: Alliance Power Audit / True Power Ranking (dashboard ⚖️ tab) — re-ranks any warzone\'s top alliances by EFFECTIVE power: total power discounted by the share of members who actually log in (last 6 days). Exposes the gap between paper power and who\'ll really show up for VS Days / KvK — a #1 alliance that\'s 50% ghosts fields half the army it advertises. Shows raw-vs-real bars, ghost %, live fighters, momentum, and how each alliance moves vs its in-game rank. Screenshot-ready to share.' },
    { date: '2026-09-22', tag: 'FEAT',  label: 'NEW: Warzone Power Race (dashboard 📊 tab) — chart the total power of the TOP 20 alliances on any warzone over ~50 days on one timeline (not just the top 5 anymore). See who\'s surging, who\'s stalling, and how the race for #1 is trending. Table shows every alliance with its window change (Δ). Reminder: LW Atlas exposes power at the alliance level (all of them) + top-5 individuals, not per-member.' },
    { date: '2026-09-22', tag: 'FEAT',  label: 'NEW: Coalition Planner (dashboard 🤝 tab) — on a landlocked server where one alliance owns the map, check off a group of alliances and instantly see if their COMBINED power passes #1. Uses authoritative per-alliance power for EVERY alliance (not just the top few). Has a "smallest coalition to pass #1" auto-finder. Your alliance flagged in blue, the reigning #1 in gold.' },
    ...
    { date: '2026-09-16', tag: 'FEAT',  label: 'NEW: Scouting Report (dashboard 🔎 tab) — a full intel dossier on ANY alliance in one screen: power trend, how many members are actually active vs. dead accounts, the R5/R4 leadership roster with exact coordinates, whales, alliance age, engagement + activity scores, and recent arrivals. Know your enemy — or vet a merge — instantly. Built on LW Atlas data fields no other tool surfaces.' },
    ...
    { date: '2026-09-15', tag: 'FEAT',  label: 'NEW: Transfer Planner (/warzone-transfer.html) — thinking of switching servers? Paste the warzones your in-game Transfer screen offers and compare them all on what actually matters: how many players are STILL active, whale depth, whether the top of the map is a real war, and — the one everyone forgets — nationality & timezone (a "busy" server that\'s 50% Indonesia runs while you\'re asleep). US/English-friendly active servers get a ★ flag. Plus a full guide to how Transfer Surge works (timing, eligibility, seats, vouchers, what you keep) and a crowdsourced map of which warzones can transfer where — it grows every time someone saves their in-game list.' },
    { date: '2026-09-14', tag: 'FEAT',  label: 'Hive Grid Manager: stronghold / hive snap. The tool now scans the whole warzone and finds the busiest high-level camps (the real contested strongholds) — tap one and the anchor jumps there, so your formation lands right on a stronghold. Gold rings on the map mark each hotspot; the anchor snaps to whole tiles.' },
    ...
    { date: '2026-09-14', tag: 'FIX',   label: 'Roster over-count guard: alliances cap at 100 members in-game, so if your roster ever shows more than 100 (the live map scan lags on members who recently left) you now get a clear ⚠️ warning with a one-click link to trim it — on EVERY tool (Landing, Freeze, Coal, City, Heat, Hive, Timeline…) and the roster manager. Every tool’s roster banner now also carries a “manage roster” link.' },
