...
No. Every tool runs in your browser. No app, no download, no build step. The suite is **11 tools** — KB Chat and Roster Extractor call our server (which hits the Claude API for you); the other 9 run entirely client-side, so your roster CSV stays on your device unless you explicitly upload it. Bookmarking `r5tools.io` is enough. If you install our (future) Discord bot, that's separate and optional.
